...
;(function(win, doc, globalName) {
  ...
  Obj.prototype = {
    ...
    showPic: function(p) {
      ...
      if (p > _this.totalPage) { // 显示章节最后提示内容
		...
		$(function(){
			...
			var toolH = $tools.outerHeight();
			$('body').click(function(){
			  var t = parseInt($header.css('top'));
			  if(t!=0){
				$header.stop(false).animate({top:0},300);
				$tools.stop(false).animate({bottom:0},300);
			  }else{
				$header.stop(false).animate({top:-toolH},300);
				$tools.stop(false).animate({bottom:-toolH},300);  
			  }
			}).css({padding:headH+'px 0 '+toolH+'px 0'});
		  });
        if (_this.readmode == 3) {
		  $(function(){
			$('#readEnd').show();
    ...
    setReadMode: function(){
      ...
      var readmodeFn = function(readmode){
        switch(readmode){
          case 1:
		    oIcon.attr('class', 'ift-juan');
            oText.text('卷轴');
            // oPagePrevTxt.attr('title','上一页').text('上一页');
            // oPageNextTxt.attr('title','下一页').text('下一页');
            oPagePrev.on('click',function(){
              _this.goPage('prev');
            });
            oPageNext.on('click',function(){
              _this.goPage('next');
            });
			oPagePrev.show();
			oPageNext.show();
          ...
          break;
          case 3:
            oIcon.attr('class', 'ift-single');
            oText.text('单页');
			oPagePrev.hide();
			oPageNext.hide();
			prevBook.show();
			nextBook.show();
            /* oPagePrevTxt.attr('title','上一章').text('上一章');
            oPageNextTxt.attr('title','下一章').text('下一章');
            oPagePrev.on('click',function(){
              var nextBook = $(this).data('prevbook');
              if(nextBook){
                location.href = nextBook;
              }else{
                layer.msg('已经是最新一章了', {btn: ['知道了']});
              }
            });
            oPageNext.on('click',function(){
              var nextBook = $(this).data('nextbook');
              if(nextBook){
                location.href = nextBook;
              }else{
                layer.msg('已经是最后一章了', {btn: ['知道了']});
              }
            }); */
          break;
        }
      ...
      // if(win.isMobile){
        var readModeMenu = $('#readModeMenu');
        var oIcon = readModeMenu.find('.hd i');
        var oText = readModeMenu.find('.hd .text');
        // var oPagePrevTxt = oPagePrev.find('.text');
        // var oPageNextTxt = oPageNext.find('.text');
		var oPagePrev = $('#pagePrev');
        var oPageNext = $('#pageNext');
		var prevBook = $('#prevBook');
		var nextBook = $('#nextBook');
        readmodeFn(readmode);
        readModeMenu.on('click', function(){
          switch(readmode){
            case 3:
              readmode = 1;
            break;
    ...
    addCollect: function(obj, data) {
      ...
      if (isFlag) {
        // initNum += 1;
        layer.msg('您已订阅了漫画: ' + data[1],{time:0});
        loaclData.splice(0, 0, data);
      }
      var iconStyle = (isFlag?'ift-lovered':'ift-collect');
      // 判断用户是否为登录用户
      var opt = __global.getLoginInfo(true);
      if (opt) {
        ...
        $.post(url, opt, function(res) {
          if (res.status === 0) {
            // 重新设置cookie, 失效时间一年
            _this.limitStore(type, loaclData, {expires: 8760, path: '/'});
            
            obj.find('i').attr('class',iconStyle);
            // obj.find('.text').text(statusTxt);
            // obj.find('.tips strong').text(initNum);
          }
        },'json')
        .fail(function(res){
          layer.msg('因网络问题导致本次操作失败!, 请重新添加。'); 
        });
      }else{
        // 重新设置cookie, 失效时间一年
        _this.limitStore(type, loaclData, {expires: 8760, path: '/'});
...
$(function(){
  ...
  // oCollectIcon.attr('class',statusTxt=='加收藏'?'ift-collect':'ift-lovered');
  oCollectIcon.attr('class',isCollectFlag?'ift-lovered':'ift-collect');
  ...
  // .find('.text').text(statusTxt);

  // 分享
  var $share = $('#share');
  var $shareBd = $share.children('.bd');
  baiduShare();
  $share.click(function(event) {
    var $this = $(this);
    if($shareBd.is(':visible')){
      // $shareBd.hide();
    }else{
      $shareBd.show();
    }
  ...
  __cr.scrollDis(function(dis){
    ...
    if(scrollTop > 72) {
      if(dis< -60 && dis < 0) {
        ...
        if(dis < 0) return;
        isUpFlag = false;
        $header.stop(false).animate({top: (-$header.outerHeight())+'px'}, 300);
        $tools.stop(false).animate({bottom: (-$tools.outerHeight())+'px'}, 300);
      }
    }else {
      isUpFlag = false;
      $header.stop(false).animate({top: 0}, 300);
