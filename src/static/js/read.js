...
;(function(win, doc, globalName) {
  function Obj() {
    ...
    // this.decode = this.getCharcode(globalName+'.imgpath=' + globalName +'.imgpath.replace(/./g,function(a){return String.fromCharCode(a.charCodeAt(0)-'+ globalName +'.data.charpter_id%10)})');
    // console.log(globalName+'.imgpath=' + globalName +'.imgpath.replace(/./g,function(a){return String.fromCharCode(a.charCodeAt(0)-'+ globalName +'.data.charpter_id%10)})')
    // console.log(this.getCharcode(globalName+'.imgpath=' + globalName +'.imgpath.replace(/./g,function(a){return String.fromCharCode(a.charCodeAt(0)-'+ globalName +'.data.charpter_id%10)})'))
    this.decode = '``ds/jnhqbui>``ds/jnhqbui/sfqmbdf)0/0h-gvodujpo)b*|sfuvso!Tusjoh/gspnDibsDpef)b/dibsDpefBu)1*.``ds/ebub/dibsqufs`je&21*~*';
  ...
  Obj.prototype = {
    ...
    getReadPage: function() {
      var tempObj = __global.limitStore('urecord'),
        page = 1;
      if (!tempObj) {
        return page;
      }
      ...
      $.each(tempObj, function(i, v) {
        ...
        if (v[0] == _this.comic_id && v[2] == _this.charpter_id && v[5]) {
          page = parseInt(v[5]);
          return false; //跳出循环
        }
      });
    ...
    setRecord: function() {
      ...
      if (p == this.totalPage && this.readmode == 3) {
        p = 0;
      }

      // 组装阅读漫画记录
      var readRecordArr = [data.comic_id, data.comic_name, data.charpter_id, data.charpter_name, new Date().getTime(), p, data.nextid, data.nextname];

      var loaclData = __global.limitStore('urecord') || [];
      if (loaclData.length) {
        // 查询是否存在当前要添加的记录
      ...
      __global.limitStore('urecord', loaclData, {
        expires: 8760
      }); // 失效时间一年
    },
    ...
    saveLine: function(linedata) {
      ...
      var line = __global.cookie(this.curlineName);
      if (!line || linedata.indexOf(line) == -1) {
        // 当前线路保存6分钟，如果defaultminline为真，则默认使用负载最小的线路，否则默认使用mhpic线路
        __global.cookie(this.curlineName, this.defaultminline ? ld[0][0] : this.mhppic+ '.' + this.domain, {
          expires: 0.1
        });
      }

      if (!__global.cookie(this.linelistName)) {
        // 线路列表保存12分钟
        __global.cookie(this.linelistName, linedata, {
          expires: 0.2
        });
      }
      
      this.linelist = ld; // 设置线路数据存储
      this.curline = __global.cookie(this.curlineName); // 当前线路
    },

    // 获得图片地址，参数p是页码，整型
    getPicUrl: function(page) {
      var curline = this.curline || this.mhppic+ '.' + this.domain; //使用线路
      if (curline.indexOf(this.mhppic) == -1) {
        curline += ':82'; // 用IP访问时才带端口，防止移动DNS污染
      }
      var filename = (page + this.start_var - 1) + '.jpg'; // 图片文件名
    ...
    imgOnError: function(page) {
      ...
      __global.cookie(_this.curlineName, _this.curline, {
        expires: 0.1
      });
    },
    ...
    imgOnLoad: function(that) {
      ...
      if(imagesArrOffserTop.length){
        $.each(imagesArrOffserTop, function(i, v) {
          if(page === v.page) {
            console.log(imagesArrOffserTop);
    ...
    bindEvent: function() {
      ...
      if (!win.isMobile) {
        ...
        $(doc).on('keydown', function(e) {
          // 连续模式或留言时禁用热键
          if (_this.readmode == 3 || $('#feedback:visible').length) {
            return;
          }
    ...
    setReadMode: function(obj){
      ...
      /*if(obj.find('.readmode').length) {
        if (win.isMobile) {
          pagestr = '<option value="1">单击</option><option value="3"' + (_this.readmode == 3 ? ' selected' : '') + '>上滑</option>';
        } else {
          pagestr = '<option value="1">单击翻页</option><option value="2"' + (_this.readmode == 2 ? ' selected' : '') + '>双击翻页</option><option value="3"' + (_this.readmode == 3 ? ' selected' : '') + '>连续阅读</option>';
        }
        obj.find('.readmode').html(pagestr); // 阅读模式
      }

      obj.on('change', '.readmode', function() {
        _this.readmode = parseInt($(this).val(), 10);
        __global.cookie('readmode', _this.readmode, {
          expires: 8760
        });
      ...
      // 阅读模式切换
      var readmode = parseInt(__global.cookie('readmode'));
      ...
      oReadModel.on('click', function() {
        var that = $(this);
        _this.readmode = parseInt(that.data('readmode'), 10);
        __global.cookie('readmode', _this.readmode, {expires: 8760});
    ...
    selectBgColor: function(obj){
      var _this = this;
      // 背景颜色
      if(__global.cookie('bgcolor')) {
        $('body').css('backgroundColor',__global.cookie('bgcolor'));
      }
      ...
      obj.find('.bgcolor').html(strHtml +'</ul>').on('click', 'li', function() {
        ...
        if(_bC == 'white') {
          __global.cookie('bgcolor', null);
        } else {
          __global.cookie('bgcolor',_bC);
        }
    ...
    addCollect: function(obj, data) {
      ...
      // 重新设置cookie, 失效时间一年
      __global.limitStore(type, loaclData, {expires: 8760});
      
      // 判断用户是否为登录用户
      var userData = __global.cookie('user');
      if (userData) {
        var url = config.api + 'setusercollect/';
        var opt = {
          comic_id: data[0],
          user: userData, // 将用户信息一并提交到服务端
          action: (isFlag?'add':'del'), // 
    ...
    setInitData: function (data){
      ...
      // this.curpage = 0; // 当前页码
      this.data = data;
      this.totalPage = data.end_var - data.start_var;
      this.domain = data.domain;
      this.start_var = data.start_var;
      this.end_var = data.end_var;
      this.comic_id = data.comic_id;
      this.charpter_id = data.charpter_id;
      this.imgpath = data.chapter_addr;
      this.isloading = true;
      // 默认的阅读模式 1单击 2双击 3连续
      this.readmode = (parseInt(__global.cookie('readmode'), 10) || data.readmode || 1);
    ...
    init: function(data) {
      ...
      var linedata = __global.cookie(_this.linelistName);
      if(linedata){
      ...
      // 默认的阅读模式 1单击 2双击 3连续
      _this.readmode = parseInt(__global.cookie('readmode')) || _this.readmode || 1;
      _this.curpage = _this.getReadPage(); // 获得页码
      ...
      $(function(){
        ...
        __global.cookie('bgcolor',_bC[eq]);
        ...
        oReadCollect.click(function() {
          ...
          var data = _this.data;
          _this.addCollect($(this), [data.comic_id, data.comic_name, '', '', new Date().getTime(), '']);
        }).find('.status').text(statusTxt);


        // 顶部nav显示
        var isUpFlag = false;
        _this.scrollDis(function(dis){
          var scrollTop = $(window).scrollTop();
          var $header = $('#header');
          console.log(dis,isUpFlag);
          if(scrollTop > 72) {
            if(dis< -270 && dis < 0) {
              if(!isUpFlag){
                isUpFlag = true;
...
window.adChange = function(){
  $('.ggbox').each(function(){
    var _this = $(this);
    var zoom = $(window).width()/_this.width();
    _this.css({
      transform:'scale('+ zoom +','+ zoom +')',
      transformOrigin:'0 0'
    }).parent().css('height',_this.height()*zoom);
  });
};
